...
inline bool contains_insensitive(std::string_view text, std::string_view pattern) {
  auto eq=[](char a,char b){return std::tolower(static_cast<unsigned char>(a))==std::tolower(static_cast<unsigned char>(b));};
...
inline std::string clean_log_detail(std::string_view raw) {
  std::string out;out.reserve(std::min<std::size_t>(raw.size(),240));
  for(char c:raw){if(out.size()==240){out+="...";break;}unsigned char u=static_cast<unsigned char>(c);out+=(u<32||u==127)?' ':c;}
  return out;
}
...
inline std::string format_bytes_rate(uint64_t bytes_per_second) {
  ...
  if(bytes_per_second>=1024ull*1024ull)o<<double(bytes_per_second)/(1024.0*1024.0)<<" MiB/s";
  else if(bytes_per_second>=1024)o<<double(bytes_per_second)/1024.0<<" KiB/s";
  else o<<bytes_per_second<<" B/s";
  return o.str();
}
} 
